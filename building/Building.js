...
class Draggable {
  ...
  _dragStart(e) {
    ...
    if (this._buildingObj.status !== BUILDING_STATUSES.ACTIVE) {
      return;
    }
...
const BUILDING_PHASES = [
  {// 0
    ...
    endTime: 0,
    endPhase: 0,
    pauseStatus: BUILDING_STATUSES.ACTIVE,
    yesItem: 0,
    noPhase: 1,
    yesPhase: 2,
    image: 0
  ...
  {// 1 - no
    ...
    endTime: 1,
    endPhase: 0,
    status: BUILDING_STATUSES.DISABLED
  },
  {// 2 - cement
    startTime: 1.2,
    endTime: 7.8,
  ...
  {// 9 - windows
    ...
    endPhase: 9,
    status: BUILDING_STATUSES.DISABLED,
    pauseStatus: BUILDING_STATUSES.DISABLED,
    image: 5
  }
];
...
class Building {
  constructor({
    ...
    this._onSetItem = this._onSetItem.bind(this);
    ...
    this._phase = 0;
    this._status = BUILDING_STATUSES.LOADING;
  ...
  _start() {
    ...
    this._setStatus(BUILDING_PHASES[this._phase].pauseStatus);
  };

  _onEnterFrame() {
    const { endTime, endPhase } = BUILDING_PHASES[this._phase];
    ...
    const { endTime: endPhaseEndTime, pauseStatus: endPhasePauseStatus, pauseStatus, image } = BUILDING_PHASES[this._phase];
  ...
  _onSetItem(item) {
    const { yesItem, yesPhase, noPhase } = BUILDING_PHASES[this._phase];
    ...
    const { startTime, status } = BUILDING_PHASES[this._phase];
  ...
  _resetAll() {
    ...
    this._setPhase(0);
    this._setImages(0);
    this._setStatus(BUILDING_STATUSES.ACTIVE);
  }
